...
def send_data_via_udp(data):
    ...
    try:
        ...
        # json_data = json.dumps(data)
         ...
         # Check if data is already in JSON string format
        if isinstance(data, dict):
            # Convert data to JSON string
            json_data = json.dumps(data)
            
        elif isinstance(data, str):
            # Data is already in JSON string format, no need to convert
            json_data = data
        else:
...
def index():
    ...
    return render_template('Arvr_demo_2.html')
...
def process_coordinates():
    ...
    if cached_result:
        ...
        send_data_via_udp(data_from_cache)
    ...
    if response.status_code == 200:
        ...
        cache.set(cache_key, {'path_size': path_size, 'instruction_size': instruction_size, 'path': coordinates, 'distance': distance, 'instruction': instruction, 'time': time}, timeout = 5)
        
        data = {'path_size': path_size, 'instruction_size': instruction_size, 'path': coordinates, 'distance': distance, 'instruction': instruction, 'time': time}  
        json_data = json.dumps(data)
        data_length = len(json_data)      
    
        data_to_send = {'data_length': data_length, 'path_size': path_size, 'instruction_size': instruction_size, 'path': coordinates, 'distance': distance, 'instruction': instruction, 'time': time}
